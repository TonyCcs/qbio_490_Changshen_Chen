...
```{r}
getwd()
setwd("/Users/chenchangshen/Desktop/qbio_490_Changshen_Chen/analysis_data")
clinic <- read.csv("/Users/chenchangshen/Desktop/qbio_490_Changshen_Chen/analysis_data/skcm_clinical_data.csv")
```
...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/Users/chenchangshen/Desktop/qbio_490_Changshen_Chen/analysis_data")) 
```
...
```{r}
#Not finishing this part because I downloaded TCGA data from blackboard
...
```{r}
write.csv(clinic, "/Users/chenchangshen/Desktop/qbio_490_Changshen_Chen/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/chenchangshen/Desktop/qbio_490_Changshen_Chen/analysis_data/brca_clinical_data.csv")
```
...
#3
head(clinic)
clinic[clinic$Tumor_Sample_Barcode == "TCGA-BF-A1PV","age_at_diagnosis"]

#4
clinic[clinic$Tumor_Sample_Barcode == "TCGA-3N-A9WB", "last_contact_days_to"]
clinic[clinic$Tumor_Sample_Barcode == "TCGA-3N-A9WB", "age_at_diagnosis"]
...
```{r Exercise 4.1}
plot(clinic$age_at_diagnosis,clinic$death_days_to, pch = 20,xlab = "Age at Diagnosis", ylab = "Days After Diagnosis")
```
...
```{r Exercise 4.2}
boxplot(clinic$gende ~ as.numeric(clinic$death_days_to))
...
```{r}
vital_mask <- !is.na(clinic$vital_status)
```
...
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis <= 35, "Young",
                                      ifelse(cleaned_clinic$age_at_diagnosis < 50, "Middle", no =                                       "Old"))
...
I cannot complete the following questions as I kept receiving errors in the KM plot section
I am pasting them below:

Error in Surv(time = cleaned_clinic$survival_time, event = cleaned_clinic$death_event) : 
  Time and status are different lengths
  
Error: object 'fit_object' not found

I would like to seek for help in-person during office hours or after class. I think there
still exists some problem within the dataframe downloaded from blackboard.
